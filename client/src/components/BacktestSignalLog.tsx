...
export default function BacktestSignalLog() {
  ...
  const pillBtn = (
    ...
    <button
      ...
      className={`font-mono text-xs uppercase tracking-wider rounded-full px-3 py-2.5 min-h-[44px] flex items-center border-none cursor-pointer transition-colors ${
        sideFilter === value
    ...
    <div data-testid="modeled-trades-dashboard" data-state={filteredOpen.some(t => t.status === 'DATA_REVIEW') ? 'data_review' : filteredOpen.length > 0 && filteredClosed.length > 0 ? 'mixed' : filteredOpen.length > 0 ? 'open' : filteredClosed.length > 0 ? 'closed' : 'empty'}>
      ...
      <div className="rounded-t-xl border border-white/10 bg-white/[0.03] px-4 py-3">
        <div className="flex items-center gap-2 mb-1">
          ...
          <p className="text-xs font-mono text-green-400/70 uppercase tracking-widest m-0">
      ...
      <div className="border-x border-white/10 grid grid-cols-3">
        {[
          ...
          >
            <p className="text-xs text-gray-500 font-mono m-0 uppercase tracking-wider">
              {s.label}
            </p>
            <p
      ...
      <div className="border-x border-white/10 border-b border-white/10 px-4 py-3">
        <p className="text-xs font-mono text-green-400/70 uppercase tracking-widest mb-2 m-0">
          Currently Tracking
        </p>
          ...
          <div className="space-y-2">
            {filteredOpen.map((t) => (
              ...
              >
                ...
                <span
                  className={`text-xs font-bold uppercase px-2 py-1 rounded-full ${sidePillClass(t.side)}`}
                >
                  {t.side}
                </span>
      ...
      <div className="border border-white/10 border-t-0 rounded-b-xl overflow-hidden mb-4">
        <p className="text-xs font-mono text-green-400/70 uppercase tracking-widest px-4 pt-3 pb-1 m-0">
          Closed — Last 14 Days
        </p>
                ...
                <thead>
                  <tr className="bg-white/5">
                    {[
                      ...
                      <th
                        key={h.label}
                        className={`px-3 py-2 text-xs text-gray-400 uppercase tracking-wider font-semibold whitespace-nowrap text-${h.align}`}
                      >
                        {h.label}
                      </th>
                    ))}
                  ...
                  {filteredClosed.map((t, i) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-center">
                        <span
                          className={`text-xs font-bold px-2 py-1 rounded-full ${outcomePillClass(t.outcome)}`}
                        >
                          {t.outcome ?? "—"}
                        </span>
                      </td>
        ...
        <div className="border-t border-white/5 px-4 py-2 flex flex-wrap items-center gap-2">
          ...
          {data_health.issues.length > 0 && (
            <span className="text-xs text-[#f0b429] flex items-center gap-1">
        ...
        {/* ─── Disclosure ──────────────────────────────────────── */}
        <p className="text-xs text-gray-600 px-4 py-2 border-t border-white/5 m-0">
